import React from "react";
import { CheckIcon } from "@heroicons/react/solid";

const Pricing = () => {
  return (
    <div name="pricing" className="w-full text-white my-24">
      <div className="h-[800px] w-full bg-slate-900 absolute mix-blend-overlay">
        Absolute
      </div>

      <div className="max-w-[1240px] mx-auto py-12">
        <div className="text-center py-8 text-slate-300">
          <h2 className="uppercae text-3xl ">Pricing</h2>
          <h3 className="text-5xl font-bold text-white py-8">
            The right price for research
          </h3>
          <p className="text-2xl">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Eaque at
            et, nostrum accusamus molestiae eveniet.
          </p>
        </div>

        <div className="grid md:grid-cols-2">
          <div className="bg-white text-slate-900 m-4 p-7 rounded-xl shadow-2xl relative h-[500px]">
            <span className="uppercase px-3 py-1 bg-indigo-200 text-indigo-900 rounded-2xl text-sm">
              Standard
            </span>
            <div>
              <p className="text-2xl font-bold py-3 flex ">
                $45
                <span className="text-sm text-slate-500 flex flex-col justify-end">
                  /mo
                </span>
              </p>
            </div>
            <p className="text-xl py-1 text-slate-500">
              Lorem ipsum dolor sit amet consectetur adipisicing.
            </p>
            <div className="text-2xl">
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <button className="w-full py-2 my-4 ">Get Started</button>
            </div>
          </div>
          
          <div className="bg-white text-slate-900 m-4 p-7 rounded-xl shadow-2xl relative h-[500px]">
            <span className="uppercase px-3 py-1 bg-indigo-200 text-indigo-900 rounded-2xl text-sm">
              Premium
            </span>
            <div>
              <p className="text-2xl font-bold py-3 flex ">
                $99
                <span className="text-sm text-slate-500 flex flex-col justify-end">
                  /mo
                </span>
              </p>
            </div>
            <p className="text-xl py-1 text-slate-500">
              Lorem ipsum dolor sit amet consectetur adipisicing.
            </p>
            <div className="text-2xl">
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <p className="flex sm:py-1 sm:text-xl lg:py-2 lg:text-2xl">
                <CheckIcon className="w-8 mr-5 text-green-600" /> Lorem, ipsum
                dolor.
              </p>
              <button className="w-full py-2 my-4 ">Get Started</button>
            </div>
          </div>

        </div>
      </div>
    </div>
  );
};

export default Pricing;
